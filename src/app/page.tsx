...
export default function Home() {
  ...
  const handleBillAmount = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(parsedValue) || parsedValue > 0) {
      let roundedValue = parseFloat(parsedValue.toFixed(2));
  ...
  return (
    <div  className="Bg flex flex-col items-center font-space-mono">

<h1 className=" pt-10 md:pt-20 md:pb-8 text-2xl header ">SPLI <br /> TTER</h1>
<div className="h-full lg:h-auto card grid grid-cols-2 mt-10 px-6 pt-6 pb-8 lg:p-7 gap-4 lg:gap-9 bg-white rounded-b-0 rounded-t-3xl lg:rounded-3xl ">
  <div className="col-span-2 md:col-span-1 lg:w-96 px-2 pt-3 lg:py-3 lg:ps-3 lg:pe-2">
    ...
    <div>
      ...
      <div className=" bg-offWhite px-3 lg:px-4 py-1 rounded-md flex flex-row justify-between items-center border-2 border-gray/0 focus-within:border-cyan">
    ...
    <div className="mt-8 lg:mt-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 text-2xl text-offWhite">
        ...
        <input value={customAmount !== undefined ? customAmount : ''} onChange={customTip} type="number" placeholder="Custom" className={!isCustom ? "py-2 bg-offWhite text-gray rounded-lg focus:text-darkCyan text-center cursor-pointer" : "py-1.5 bg-offWhite text-gray rounded-lg focus:text-darkCyan text-center custom"} />
    ...
    <div className="mt-8 lg:mt-10">
      ...
      <div className={isSelected && isFilled && peopleCount === undefined || isCustom && isFilled && peopleCount === undefined ? "bg-offWhite px-4 py-1.5 rounded-md flex flex-row justify-between items-center zeroBorder" : "bg-offWhite px-4 py-1.5 rounded-md flex flex-row justify-between items-center border-2 border-gray/0 focus-within:border-cyan"}>
        ...
        <input value={peopleCount ?? ''} onChange={handlePeople} className="!w-52 lg:!w-auto bg-offWhite text-2xl text-end border-0 border:border-0 text-darkCyan" min='0' type="number" placeholder="0" />
      </div>
    </div>
  </div>
